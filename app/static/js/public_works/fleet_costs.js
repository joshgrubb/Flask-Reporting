...
function initTimeSeriesChart(data, interval) {
    ...
    formattedData.sort((a, b) => a.period - b.period);
    ...
    // Create datasets
    const laborCosts = formattedData.map(item => item.laborCost);
    const materialCosts = formattedData.map(item => item.materialCost);
    const totalCosts = formattedData.map(item => item.totalCost);
    const workOrders = formattedData.map(item => item.workOrders);
    ...
    /**
     * Initialize the time series table
     * @param {Array} data - The time series data
     */
